...
assert(taskWindowSource.includes('function focusObjectTaskDockEntry(kind, nodeId)'), 'Dock should be able to return focus to a minimized task restore button');
...
assert(taskWindowSource.includes("minimizeTaskWindow({ focusDockEntry: true })"), 'Minimize button should move focus to the dock restore button');
...
assert(taskWindowSource.includes('function focusTaskWindowLauncher(kind = null, nodeId = null)'), 'Close flow should be able to return focus to the task-window launcher');
assert(taskWindowSource.includes("closeTaskWindow({ focusReturn: true })"), 'User close/Escape should request focus return');
...
assert(taskWindowSource.includes("document.body.classList.toggle('fluid-basis-task-open', !minimized && !taskWindow.hidden)"), 'Fluid Basis minimize should release the large open-layout body state');
assert(taskWindowSource.includes('if (minimized) closeTabletFluidBottomDock();'), 'Fluid Basis minimize should close the tablet bottom trace dock');
...
assert(styles.includes('body.fluid-basis-task-open .task-window.task-window-fluid-active:not(.task-window-minimized)'), 'Fluid Basis landscape sizing must not override minimized sizing');
assert(indexHtml.includes('body.fluid-basis-task-open .task-window.task-window-fluid-active:not(.task-window-minimized)'), 'Critical CSS must protect minimized Fluid Basis sizing before async CSS loads');
