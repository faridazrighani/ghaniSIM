...
const migratedElevation = normalizeProps({
    elevation: 6,
    tankHeight: 6,
    liquidLevel: 3,
    hll: 5,
...
assertClose('legacy inlet nozzle offset migration', migratedElevation.inletNozzleElevation, 9, 0.001);
assertClose('legacy outlet nozzle offset migration', migratedElevation.outletNozzleElevation, 7, 0.001);
assertClose('legacy transmitter offset migration', migratedElevation.tLevelElev, 9, 0.001);
assert(migratedElevation._legacyTankElevationFieldMigrated === true, 'Expected legacy elevation field migration flag');
assert(warningIncludes(evaluateTank(migratedElevation), 'converted from tank-relative offsets'), 'Expected legacy elevation review warning');

vm.runInContext(`
(() => {
    const props = ${JSON.stringify(migratedElevation)};
    acknowledgeTankElevationBasis(props);
    if (props._legacyTankElevationFieldMigrated) throw new Error('Elevation review flag should be cleared after acknowledgement');
    if (!props._tankElevationBasisReviewed) throw new Error('Reviewed marker should be set after acknowledgement');
    const safety = evaluateTankPressureSafety(props, {});
    if (safety.warnings.some(item => item.includes('converted from tank-relative offsets'))) {
        throw new Error('Acknowledged elevation basis should not keep the legacy warning');
    }
})()
`, context);
