...
assert(licTrace.status === 'Waiting for tank/vessel level attachment', 'Unattached LIC should wait for tank/vessel attachment');
assert(licTrace.warnings.some(item => item.includes('not attached to a tank/vessel')), 'Unattached LIC should warn about missing level target');
...
globalModel['LIC-100'].props.attachedTo = 'TK-100';
`, context);
const licTankTrace = vm.runInContext(`buildInstrumentCalculationTrace('LIC-100', globalModel, connections)`, context);
...
const usLicHtml = vm.runInContext(`renderInstrumentCalculationTraceReport(buildInstrumentCalculationTrace('LIC-100', globalModel, connections))`, context);
assert(usLicHtml.includes('ft'), 'US LIC trace should show level/elevation values as ft');
