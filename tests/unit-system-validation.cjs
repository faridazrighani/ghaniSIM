...
assert(vm.runInContext(`formatDisplayUnitValueByUnit(25, 'mbar g', 6, 'tankDesignPressure', 'Tank Design Pressure')`, context) === '0.362594 psig', 'US mbar gauge fields should scale from mbar to psi');
assert(vm.runInContext(`formatDisplayUnitValueByUnit(5, 'mbar vacuum', 6, 'designVacuum', 'Design Vacuum')`, context) === '0.072519 psi', 'US mbar vacuum fields should scale from mbar to psi');
assertClose(
    'US psig display back to mbar',
    vm.runInContext(`getInternalValueFromDisplay(0.362594344325, 'tank', 'tankDesignPressure', 'Tank Design Pressure', 'mbar g')`, context),
    25,
    1e-9
);

vm.runInContext(`setUnitStandard(UNIT_STANDARD_SI, { markDirty: false })`, context);
assert(vm.runInContext(`formatDisplayUnitValueByUnit(25, 'mbar g', 3, 'tankDesignPressure', 'Tank Design Pressure')`, context) === '2.500 kPa g', 'SI mbar gauge fields should scale from mbar to kPa');

vm.runInContext(`setUnitStandard(UNIT_STANDARD_METRIC, { markDirty: false })`, context);
assert(vm.runInContext(`formatDisplayUnitValueByUnit(25, 'mbar g', 3, 'tankDesignPressure', 'Tank Design Pressure')`, context) === '25.000 mbar g', 'Metric tank mbar fields should retain mbar units');
