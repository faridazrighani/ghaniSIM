...
assert(styles.includes('min-height: 68px;'), 'Desktop ribbon should reserve final compact toolbar height to reduce CLS');
assert(styles.includes('min-height: 54px;'), 'Toolbar palette should reserve compact icon-group height before JavaScript hydration');
...
assert(indexHtml.includes('class="toolbar-palette" id="toolbarPalette"'), 'Toolbar palette should have static first-paint content to avoid hydration CLS');
assert(indexHtml.includes('id="toolbarPalette" role="toolbar"'), 'Static toolbar shell should preserve accessible toolbar semantics');
